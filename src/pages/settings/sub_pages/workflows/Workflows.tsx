...
const createTriggerNode = (event: string = TRIGGER_EVENT_OPTIONS[0].value): BuilderNode => {
...
const createActionNode = (actionType: string = ACTION_TYPES[0].value, notes = ""): BuilderNode => {
...
const mapWorkflowToDraft = (workflow: Workflow & { current_version?: any }): WorkflowDraft => {
  ...
  let nodes: BuilderNode[] = Array.isArray(version?.nodes) ? version.nodes.map(normalizeNode) : [];
...
function Workflows() {
  ...
  const nodeIcon = (_type: NodeType) => faDiagramProject;
            ...
            {actions.map((action) => (
              <Card key={action.id} className="p-3 space-y-3 border-dashed">
                <div className="flex items-center justify-between">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => handleRemoveAction(action.id)}>
                    Remove
                  </Button>
                </div>
                <div className="space-y-2">
